...
def parse_feed(content: bytes, source: str, discovered_by: str, query: str) -> list[Item]:
    ...
    for entry in root.findall(".//item"):
        ...
        if title and link:
            items.append(Item(title, link, source, published_text, published_at, discovered_by, query, summary))
...
def collect_items(days: int) -> tuple[list[Item], list[str]]:
    ...
    for query in DIRECT_SEARCH_QUERIES + SITE_QUERIES:
...
def dedupe(items: list[Item]) -> list[Item]:
    ...
    unique: list[Item] = []
    for item in items:
        keys = {normalize_url(item.url), title_fingerprint(item.title), *entity_keys(item.title)}
...
def build_report(items: list[Item], errors: list[str], today: dt.date) -> str:
    ...
    lines = [f"{REPORT_TITLE} - {today.isoformat()}", "", "\u4eca\u65e5\u91cd\u70b9\u7ebf\u7d22"]
...
def format_item(index: int, item: Item, compact: bool = False) -> list[str]:
    lines = [
        f"{index}. {item.title}",
        f"   \u6765\u6e90\uff1a{item.source}\uff1b\u53d1\u73b0\u65b9\u5f0f\uff1a{item.discovered_by}\uff1b\u53ef\u4fe1\u5ea6\uff1a{credibility(item)}",
    ]
    if item.published:
